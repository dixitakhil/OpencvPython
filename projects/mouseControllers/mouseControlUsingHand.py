...
while(True):
    ...
    if len(contours)>0:
        ...
        if abs(distanceTransversedX)>2 or abs(distanceTransversedY)>2:
            ...
            m.move(x-distanceTransversedX*6,y-distanceTransversedY*6)

           #  x,y=pyautogui.position()
            # pyautogui.moveTo(x-distanceTransversedX*6,y-distanceTransversedY*6)

        #Click function
        # if len(contoursr)>0 and len(contours)>0:
